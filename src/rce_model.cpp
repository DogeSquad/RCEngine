...
void RCEModel::createVertexBuffer(const std::vector<Vertex>& vertices)
{
	...
	vertexBuffer = std::make_unique<RCEBuffer>(
		rceDevice,
		vertexSize,
		vertexCount,
	...
	rceDevice.copyBuffer(stagingBuffer.getBuffer(), vertexBuffer->getBuffer(), bufferSize);
}
void RCEModel::createIndexBuffer(const std::vector<uint32_t>& indices)
{
	...
	indexBuffer = std::make_unique<RCEBuffer>(
		rceDevice,
		indexSize,
		indexCount,
	...
	rceDevice.copyBuffer(stagingBuffer.getBuffer(), indexBuffer->getBuffer(), bufferSize);
}
